...
const getBrandsSellers = unstable_cache(
  async () => prisma.seller.findMany({
    ...
    select: { ...sellerCardSelect, _count: { select: { products: true } } },
    orderBy: { createdAt: "desc" },
    take: 2,
  }),
...
const getTrendingProducts = unstable_cache(
  async (page: number, itemsPerPage: number) => prisma.product.findMany({
    where: { isDeleted: false, isPublished: true, seller: { verification: { kycStatus: { in: ["auto_approved", "approved"] }, bankVerified: true } } },
    select: productCardSelect,
    orderBy: { createdAt: "desc" },
...
export default async function HomePage({ searchParams }: PageProps) {
  ...
  if (userProfile) wishlistIds = (await redis.smembers(`wishlist:${(userProfile as any).id}`)) || [];
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-vl-surface font-vl-body text-vl-ink">
      <HomeHeader userProfile={userProfile as any} cartCount={cartCount} sellerHref={sellerHref} />
      <main className="pb-24 md:pb-0">
        <HomeHero />
        <HomeCategoryGrid />
        {spotlightProducts.length > 0 ? (
          <section className="vl-section-shell mt-16 sm:mt-24">
            <div className="flex items-end justify-between gap-4"><div><p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-vl-secondary">Trending now</p><h2 className="font-vl-heading text-2xl font-bold tracking-[-0.04em] text-vl-ink sm:text-3xl">The pieces everyone is saving</h2></div><Link href="/products" className="hidden rounded-vl-control px-3 py-2 text-sm font-semibold text-vl-muted transition hover:bg-vl-card hover:text-vl-primary sm:inline-flex">Shop all</Link></div>
            <div className="mt-6 grid gap-4 md:grid-cols-2">{spotlightProducts.map((product) => <article key={product.id} className="group grid overflow-hidden rounded-vl-card bg-vl-ink text-white sm:grid-cols-[0.9fr_1.1fr]"><div className="relative min-h-[280px] bg-vl-border"><Image src={product.images?.[0]?.url || "/placeholder.jpg"} alt={product.name} fill sizes="(max-width: 640px) 100vw, 30vw" className="object-cover transition duration-500 group-hover:scale-105" /><WishlistIconButton productId={product.id} isLoggedIn={!!session?.user} initialIsWishlisted={wishlistIds.includes(product.id)} /></div><div className="flex flex-col justify-end p-6 sm:p-7"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-white/55">{product.seller.businessName}</p><h3 className="mt-3 font-vl-heading text-2xl font-bold leading-tight tracking-[-0.04em]">{product.name}</h3><div className="mt-5 flex items-baseline gap-3"><span className="font-vl-heading text-2xl font-bold">{formatPrice(product.price)}</span><span className="text-sm text-white/45 line-through">{formatPrice(product.price * 1.4)}</span></div><Link href={`/products/${product.id}`} className="mt-6 inline-flex min-h-11 w-fit items-center rounded-vl-control bg-vl-primary px-4 text-sm font-semibold text-white transition hover:bg-vl-primary-strong">Shop this piece</Link></div></article>)}</div>
          </section>
        ) : null}
