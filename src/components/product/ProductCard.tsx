  ...
  return (
    <div className="group relative flex flex-col bg-vl-card border border-vl-border rounded-vl-card overflow-hidden cursor-pointer transition-all duration-vl-standard hover:shadow-vl-medium hover:-translate-y-1">
      ...
      <Link href={`/products/${product.id}`} className="relative aspect-square overflow-hidden bg-vl-surface flex-shrink-0 block">
        ...
        <div className="absolute top-1.5 left-1 z-10 flex items-center gap-1.5 rounded-lg px-1.5 py-1">
          {product.seller.storeLogo ? (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img
              src={product.seller.storeLogo}
              alt=""
              className="w-5.5 h-5.5 rounded-full object-cover shrink-0 border border-white shadow-md"
            />
          ) : (
            <div className="w-5.5 h-5.5 rounded-full bg-white/95 flex items-center justify-center text-[9px] font-black text-vl-primary shrink-0 select-none shadow-md">
              {product.seller.businessName[0].toUpperCase()}
            </div>
          )}
          <span className="text-[10px] font-extrabold text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)] truncate max-w-[90px]">
